...
class Check:
    ...
    def sheet(self, work: data.Work, w_i: int, sheet: data.Sheet, s_i: int):
        ...
        for r in sheet.data:
            ...
            # 这里只是打印一下头部信息
            common.out_jindu(w_i, len(self.works.works), work.name,
                             sheet.name, s_i, len(work.data), r_i,
                             len(sheet.data), self.log_path)

            common.out_title(w_i, len(self.works.works), work.name, sheet, s_i,
                             r_i, len(work.data), self.log_path)

            # 这里开始是审核
            # 每个论坛的审核都会返回一个结果
            resault_1 = common.check_forum(self.works, w_i, s_i, r_i, 1,
                                           self.log_path)
            ...
            resault_2 = common.check_forum(self.works, w_i, s_i, r_i, 2,
                                           self.log_path)
            ...
            resault_3 = common.check_forum(self.works, w_i, s_i, r_i, 3,
                                           self.log_path)
    ...
    def run(self):
        ...
        for w in self.works.works:
            ...
            _work = self.work(w, w_i)

            history_list.append(_work.history.to_json())
            w_i += 1

        history_path = os.path.join(self.target_path, 'history.json')
